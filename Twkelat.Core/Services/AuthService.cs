...
namespace Twkelat.BusinessLogic.Services
{
	public class AuthService : IAuthService
	{
		...
		public async Task<AuthModelResponse> RegisterAsync(RegisterModel registerModel)
		{
			...
			var emailMsg = $"Dear {user.Name}\n Welcome in Tawkelat Application";
			if (user.Email != null)
			{
		...
		public async Task<bool> CheckRequest(CheckRequestDTO checkRequest)
		{
			...
			var userTempCode = _unitOfWork.TwkelateChain.GetUserTempCode(user.Id);

			if (string.IsNullOrWhiteSpace(userTempCode)) return false;

			if (userTempCode == checkRequest.Key) return true;
			return false;
		}
		public async Task<bool> ChangePassword(ChangeCodeRequestDTO codeRequest)
		{
			var user = await _unitOfWork.User.GetbyCivilIdAsync(codeRequest.CivilId);
			var result = await _userManager.ChangePasswordAsync(user, codeRequest.OldCode, codeRequest.NewCode);
			if (result.Succeeded)  return true;
			return false;
		}
		public async Task<bool> GenerateCodeRequest(string civilId)
		{
			var user =await  _unitOfWork.User.GetbyCivilIdAsync(civilId);
			var tempCode = new UsersTempCode()
			{
				CivilID = civilId,
			...
			_unitOfWork.TwkelateChain.AddUserTempCode(tempCode);
			if (await _unitOfWork.Save())
			{
				if(!string.IsNullOrWhiteSpace(user.Email))
					_emailSender.Send($"Your Code is {tempCode.Key}", user.Email);
